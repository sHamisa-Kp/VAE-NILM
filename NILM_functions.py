...
def create_model(model, config, width, optimizer):
    ...
    if model == "VAE":
        ...
        if config == 0:
            ...
            dconv_seq5 = conv_block_seq_res(deconv1, int(start_filter_num * 7.5), kernel_size, 1, "dconv_seq5",
                                            In=False)
            ...
            dconv_seq7 = conv_block_seq_res(deconv3, int(start_filter_num * 4.5), kernel_size, 1, "dconv_seq7",
                                            In=False)
            ...
            dconv_seq9 = conv_block_seq_res(deconv5, int(start_filter_num * 1.5), kernel_size, 1, "dconv_seq9",
                                            In=False)
        ...
        if config == "fixe_filter":
            ...
            ###############################################################################
            # New for conditional VAE
            dconv_seq4 = conv_block_seq_res_fixe(reshape1, start_filter_num, kernel_size, 1, "dconv_seq4", In=False,
                                                 ResCon=False)
            dconc5 = tf.keras.layers.concatenate([dconv_seq4, conv_seq7], name="dconc5")
        ...
        # 1D Conv
        model.add(
            tf.keras.layers.Conv1D(8, 4, activation="linear", input_shape=(sequence_len, 1), padding="same", strides=1))
        ...
        conv_layer_1 = tf.keras.layers.Conv1D(filters=30, kernel_size=10, strides=1, padding="same", activation="relu")(
            reshape_layer)
        conv_layer_1 = tf.keras.layers.Dropout(0.5)(conv_layer_1)
        conv_layer_2 = tf.keras.layers.Conv1D(filters=30, kernel_size=8, strides=1, padding="same", activation="relu")(
            conv_layer_1)
        conv_layer_2 = tf.keras.layers.Dropout(0.5)(conv_layer_2)
        conv_layer_3 = tf.keras.layers.Conv1D(filters=40, kernel_size=6, strides=1, padding="same", activation="relu")(
            conv_layer_2)
        conv_layer_3 = tf.keras.layers.Dropout(0.5)(conv_layer_3)
        conv_layer_4 = tf.keras.layers.Conv1D(filters=50, kernel_size=5, strides=1, padding="same", activation="relu")(
            conv_layer_3)
        conv_layer_4 = tf.keras.layers.Dropout(0.5)(conv_layer_4)
        conv_layer_5 = tf.keras.layers.Conv1D(filters=50, kernel_size=5, strides=1, padding="same", activation="relu")(
            conv_layer_4)
...
def EpD_metric(x_pred, x_test, sampling=6):
    ...
    for i in range(x_pred.shape[0]):
        ...
        EpD[i] = np.mean(np.abs(np.sum(x_pred[i, 0:N_days * sPerDay].reshape(N_days, -1), axis=-1) - np.sum(
            x_test[i, 0:N_days * sPerDay].reshape(N_days, -1), axis=-1))) * sampling / 3600
...
def reconstruct(y, width, strides, merge_type="mean"):
    ...
    for i in range(y.shape[0]):
        for d in range(depth):
            yr[i * strides + (d * strides):i * strides + ((d + 1) * strides), d] = y[i, d * strides:(d + 1) * strides,
                                                                                   0]
